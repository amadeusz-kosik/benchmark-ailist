...
object AIListBuilder {
  ...
  def buildSpeedOptimized(configuration: Configuration, sourceIntervals: Array[Interval]): Array[AIList] = {
    ...
    var intervals = sourceIntervals
      .sorted(Ordering.by[Interval, (Long, Long)](i => (i.from, i.to)))
      .toBuffer

    val results = ArrayBuffer[AIList]()

    while(intervals.nonEmpty) {
      val newComponent = ArrayBuffer[Interval]()
      val leftovers = ArrayBuffer[Interval]()
      ...
      intervals = leftovers
      results += AIList(newComponent.toArray)
    }
